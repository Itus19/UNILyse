...
def extract_course_info(soup):
    try:
        ...
        if faculty:
            ...
            # Vérifie si le texte contient "Faculté" et l'extrait correctement
            if "Faculté" in text:
        ...
        # Crédits (cherche la chaîne "Crédits:" dans la page)
        credits_text = soup.find(string=lambda text: text and "crédits:" in text.lower())
        if credits_text:
            course_info['Crédits'] = credits_text.split(":")[-1].strip().split()[0]

        # Semestre
        content = soup.get_text().lower()
        printemps_present = "printemps" in content
        automne_present = "automne" in content
        ...
        # Cursus
        if "baccalauréat" in content:
            course_info['Cursus'] = "Bachelor"
        elif "maîtrise" in content:
